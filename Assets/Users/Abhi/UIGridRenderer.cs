...
public class UIGridRenderer : Graphic
{
    ...
    private float cellWidth, cellHeight, width, height;
    public UILineRenderer lineRenderer;

    protected override void Start()
    {
        InvokeRepeating(nameof(UpdateGraph), .0001f, .0001f);
    ...
    {
        if(GetComponentInChildren<UILineRenderer>())
        {
            GetComponentInChildren<UILineRenderer>().enabled = false;
            GetComponentInChildren<UILineRenderer>().enabled = true;
        }        
    }
